        ...
        <div>
          {personalPosts.map((post) => (
            <div key={post._id} className="bg-light w-100 rounded-4 p-3 border border-secondary mt-4 mb-4">
              <p className="fw-bold">
                <img src={post.user.image} alt="" style={{ width: '100px', height:"100px" }} className="rounded-circle me-3" />
                <span className="fs-2">{post.user.name !== "" ? (post.user.name + " " + post.user.surname) : (post.user.username)}-</span> {post.user.title}
              ...
              {postCommentsVisibility[post._id] && (
                <PostsComments
                  postId={post._id}
                  onClick={() => { handleClose(); }}
                  handleClose={() => setPostCommentsVisibility(prevState => ({ ...prevState, [post._id]: false }))}
                />
              )}
